...
p_down <- numeric() # proportion of downtime in simulation
for (j in 1:1000) { # simulate 1000 years
 ...
 p_down[j] <- mean(down) #proportions of downtime with simulation of 1000 years
}
  

hist(p_down)
abline(v = mean(p_down), lty = 2, col = "blue")

mean(p_down)

annual_lost <- 365*mean(p_down)*1000
annual_lost
```
...
x <- seq(-5,5,001)
 y <- ifelse (x < 0, x/25+1/5, -x/25+1/5)
plot(x, y, type = "l")
```
